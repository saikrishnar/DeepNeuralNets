function [ol] = fp_av_mnist(X,Wt,b,nl,fl,nh,a_tanh,b_tanh,wtl,btl,berp,Nin)
...
bervm =  binornd(1,berp(1),[Nin nl(1)]);
X = X.*bervm;
...
% added on 28/11/14 for drop out with bernoulli probability "berp"
bervm =  binornd(1,berp(2),[Nin nl(2)]);
olm = olm.*bervm;
...
for i = 2:nh
    ...
    % added on 28/11/14 for drop out with bernoulli probability "berp"
    bervm =  binornd(1,berp(i+1),[Nin nl(i+1)]);
    olm = olm.*bervm;
