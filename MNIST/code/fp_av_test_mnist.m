function [ol] = fp_av_test_mnist(X,Wt,b,nl,fl,nh,a_tanh,b_tanh,wtl,btl,berp,Nin)
...
Xm = [gpuArray(ones(Nin,1)) X]; clear X; % ( Nin x (din+1))
rWt = reshape(Wt(1,wtl(1):wtl(2)-1),nl(2),nl(1))';
ac = Xm*[b(btl(1):btl(2)-1);rWt*berp(1)];
...
switch fl(1)
    ...
    case 'L'
        olm = ac;
    otherwise
        disp('error: please enter a valid output function name (N/S/R/L)');
        return;
end
...
for i = 2:nh
    ...
    Xm = [gpuArray(ones(Nin,1)) olm]; % ( Nin x (din+1))
    clear olm;
    
    rWt = reshape(Wt(1,wtl(i):wtl(i+1)-1),nl(i+1),nl(i))';
    ac = Xm*[b(1,btl(i):btl(i+1)-1);rWt*berp(i)]; % average the outgoing weights ( added on 02/12/2014 for dropout testing)
